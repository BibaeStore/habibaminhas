    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 divide-y divide-[var(--admin-border)] md:grid-cols-3 md:divide-x md:divide-y-0">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="px-6 py-5">
              <div className="h-3 w-28 animate-pulse rounded bg-[var(--admin-border)]" />
              <div className="mt-3 h-8 w-20 animate-pulse rounded bg-[var(--admin-border)]" />
              <div className="mt-2.5 h-3 w-14 animate-pulse rounded bg-[var(--admin-border)]" />
            </div>
          ))}
        </div>
      </AdminCard>
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-[120px] animate-pulse rounded-[var(--admin-radius)] bg-[var(--admin-border)]" />
        ))}
      </div>
    </div>
  );
...
export function DashboardClient() {
  // Shared cache keys — same data the Orders/Products/Customers pages use,
  // so navigating between dashboard and those pages never refetches
  const { data: allOrders, isPending: ordersPending } = useQuery({
    queryKey: ["orders"],
    queryFn: () => getOrders(),
  });
  const { data: allProducts, isPending: productsPending } = useQuery({
    queryKey: ["products"],
    queryFn: () => getProducts(),
  });
  const { data: orderStats, isPending: orderStatsPending } = useQuery({
    queryKey: ["order-stats"],
    queryFn: () => getOrderStats().catch(() => null),
  });
  const { data: customerStats, isPending: customerStatsPending } = useQuery({
    queryKey: ["customer-stats"],
    queryFn: () => getCustomerStats().catch(() => null),
  });

  const isPending = ordersPending || productsPending || orderStatsPending || customerStatsPending;
  ...
  const hasNoData = !orderStats && !customerStats && products.length === 0;
