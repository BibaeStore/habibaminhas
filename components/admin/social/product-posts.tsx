...
/**
 * One product stream — carousel or static — rendered from the same component.
 *
 * Parametrised rather than copied. Two 840-line pages would have drifted apart within a week,
 * and every fix to one would silently miss the other. `/admin/social` renders it as the
 * carousel; `/admin/social/static` renders it as the static stream. The only differences are
 * which rows are fetched and which rotation is shown, both of which follow from `stream`.
 *
...
export function ProductPostsPage({ stream }: { stream: LogStream }) {
  const { act, pending, notice, setNotice } = useAct();
  // Keyed by stream, so switching tabs does not show the other stream's rows from cache.
  const { data, error } = useQuery({
    queryKey: ["social-posts", stream],
    queryFn: () => load(stream),
  });
    ...
    <div>
      ...
      {tab === "upcoming" && (
        <UpcomingTab
          ...
          pending={pending}
          onAct={act}
          onPostNow={() => act(() => triggerPostNow(undefined, stream), describeRun)}
        />
      )}
